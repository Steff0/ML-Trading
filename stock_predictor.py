...
class MilanoStockPredictionAI:
    ...
    def __init__(self, data_dir="milan_data"):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    os.path.join(data_dir, "milan_ai.log"), encoding="utf-8"
                ),
    ...
    def _save_correlations(self, primary_symbol, correlations):
        ...
        try:
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO market_correlations
                (primary_symbol, correlated_market, correlation_coefficient,
                correlation_type, calculation_date, data_period, significance_level)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    primary_symbol,
                    market,
                    corr_data["spearman"]["corr"],
                    "spearman",
                    today,
                    90,
                    corr_data["spearman"]["p_value"],
                ),
            )
